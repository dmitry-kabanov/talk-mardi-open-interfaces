...
\usetheme[
  numbering=fraction,    % Slide numbers: {none, *counter*, fraction}
...
% -----------------------------------------------------------------------------
% Title info
\title{Software package \emph{MaRDI Open Interfaces}\\for increased interoperability in computational science}
\author{Dmitry I.\ Kabanov, Stephan Rave, Mario Ohlberger}
\institute{Mathematics Münster, University of Münster, Germany}
\date{GAMM 96\textsuperscript{th} Annual Meeting, Stuttgart, 17 March 2026}
...
\begin{frame}{Typical situation in computational science}
  \begin{itemize}
    ...
    \item<2-> \alert{Two problems:} \textbf{different interfaces} and \textbf{different languages}
  \end{itemize}
\end{frame}

\begin{frame}{Pairwise solution: connect all languages and solvers}
...
\begin{frame}{Indirect solution}
  \begin{columns}[t]
    \column{0.5\textwidth}
    \textbf{Provide generic interfaces} for typical numerical problems,
    employing the software principle
    \emph{``program against an interface, not an implementation''}

    \column{0.5\textwidth}
    \onslide<2->
    \textbf{Automate data marshalling} between languages
...
\begin{frame}{Indirect solution: \emph{MaRDI Open Interfaces}}
  \begin{minipage}{0.45\textwidth}
    To solve both problems simultaneously:
    \begin{itemize}
      \item Provides a mediator library \texttt{OIF} that:
            \begin{itemize}
...
\begin{frame}{Software details: Components}
  \centering
  \vspace{-1em}
  \includegraphics[scale=0.11]{arch}
...
\begin{frame}{Software details: Currently supported languages, data types}
  \begin{itemize}
    ...
    \item Data types:
          \begin{itemize}
            \item \texttt{OIF\_TYPE\_INT}: 32-bit integers
            \item \texttt{OIF\_TYPE\_F64}: 64-bit floating-point numbers
            \item \texttt{OIF\_TYPE\_ARRAY\_F64}: arrays of 64-bit floating-point numbers
            \item \texttt{OIF\_TYPE\_STRING}: strings
            \item \texttt{OIF\_TYPE\_CALLBACK}: callback functions
            \item \texttt{OIF\_TYPE\_USER\_DATA}: user-data objects of volatile type
            \item \texttt{OIF\_TYPE\_CONFIG\_DICT}: option-value pairs
          \end{itemize}
          \onslide<3->
    \item Interfaces (numerical problems): time integration, optimization
          (work-in-progress)
  \end{itemize}
\end{frame}

\begin{frame}{Software details: function calls, quality assurance}
  \begin{itemize}
    \item Functions arguments are read-only or mutable (passed by reference),
          or return values (copied)
          \onslide<2->
    \item On implementation side, functions are invoked via
          ...
          \onslide<3->
          \vspace{1em}
    \item Builds via CMake
    \item Testing: CTest + Google Test, pytest, Testing.jl
    \item Continuous integration on GitHub Actions
  \end{itemize}
\end{frame}
...
\begin{frame}{Software details: Arrays are mutable arguments}
  \centering
...
% -----------------------------------------------------------------------------
\section{Example 1: interface for optimization for Julia users}

\begin{frame}[fragile]{Example 1: Minimizing the Rosenbrock function}
  \begin{columns}
    \begin{column}{\dimexpr0.55\textwidth-2\tabcolsep}
      We seek $x^* \in \mathbb R^n$ of the Rosenbrock function\footnotemark[1]:
      \begin{align*}
        x^* = \argmin_{x\in\mathbb R^n}
        \sum_{i=1}^{n-1} a {\left( x_{i+1} - x_i^2 \right)}^2 +
        {\left( 1 - x_i \right)}^2 + b
      \end{align*}
      with an exact minimum at \( x^* = [1, \ldots, 1]\).
    \end{column}
    \begin{column}{\dimexpr0.45\textwidth-2\tabcolsep}
      \centering
      \includegraphics[scale=0.3]{rosenbrock_contour}
    \end{column}
  \end{columns}
  \footnotetext[1]{Rosenbrock, H.H. (1960). ``An automatic method for finding
    the greatest or least value of a function''}
\end{frame}

\begin{frame}[fragile]{Example 1: Julia user calls \textsc{Ipopt.jl}}
  \begin{columns}
    \begin{column}{\dimexpr0.7\textwidth}
      \begin{minted}[autogobble, beameroverlays, escapeinside=||]{Julia}
    x0 = [3.14, 2.72, 42.0, 9.81, 8.31]
    user_data = (10, -1)

    s = Optim.Self("ipopt_jl")
    Optim.set_initial_guess(s, x0)
    Optim.set_user_data(s, user_data)
    Optim.set_objective_fn(s, rosenbrock_fn)
  ...
  \hrule
  \texttt{%
  Message: The algorithm converged to a stationary point, local optimal solution, could not find directions for improvement, or otherwise completed its search without global guarantees.\\
  x = [1.000000000670358, 1.0000000013562615, 1.0000000027223896, 1.0000000053026934, 1.000000009561287]
  }
\end{frame}

\begin{frame}[fragile]{Example 1: Julia user calls \textsc{SciPy}}
  \begin{columns}
    \begin{column}{\dimexpr0.7\textwidth}
      \begin{minted}[autogobble, beameroverlays, escapeinside=||]{Julia}
    x0 = [3.14, 2.72, 42.0, 9.81, 8.31]
  ...
  x = [1.000000001123758, 1.0000000001685387, 1.0000000001452347, 1.0000000005750314, 1.0000000013416621]
  }
\end{frame}

% -----------------------------------------------------------------------------
\section{Example 2: interface for initial-value problems}

\begin{frame}[fragile]{Example 2: Solving a stiff system}
  \begin{columns}[t]
    \column{\dimexpr0.4\textwidth}
...
\begin{frame}[fragile]{Example 2: Solving a stiff system}
  ...
  \begin{columns}
    ...
    \onslide<6->{%
      \textbf{\color{CGreenD}Success}\\
      More efficient solution using \texttt{OrdinaryDiffEq.jl} in 2 seconds.
    }
  \end{columns}
\end{frame}
...
\begin{frame}{Performance study}
  \begin{minipage}{\dimexpr0.6\textwidth}
    ...
    \end{itemize}
  \end{minipage}\hfill
  \begin{minipage}{\dimexpr0.4\textwidth}
...
\begin{frame}
  ...
  \begin{minipage}{0.618\textwidth}
    \begin{itemize}
      ...
      \item \texttt{pip install openinterfaces}
            \onslide<3->
      \item Published in \emph{Journal of Open Research Software}, 2025 [DOI:\@10.5334/jors.569]
      \item Preprint: \url{https://arxiv.org/abs/2504.03628}
    \end{itemize}
  \end{minipage}%
  \hfill
  \begin{minipage}{0.282\textwidth}
    \onslide<1->
    \includegraphics[scale=0.15]{qr/mardi-oif-github.png}
    \\Repository\\
    \\
    \includegraphics[scale=0.15]{qr/mardi-oif-paper.png}
    \\Paper\\
  \end{minipage}
\end{frame}
...
\begin{frame}{Summary}
  \begin{itemize}
    \item \emph{MaRDI Open Interfaces} provides a way
          to avoid error-prone and time-consuming
          code modifications and testing for computational projects
    \item Currently there is support for three programming languages: C, Julia, and Python
    \item Currently there is for two interfaces: initial-value problems for ODEs
          and nonlinear optimization (WIP)
    \item Cross-language interoperability does not sacrifice performance,
          as arrays are shared
          \onslide<2->
    \item We are open for collaborations and contributions!
  \end{itemize}
\end{frame}
...
\begin{frame}
  \frametitle{(BU) Performance study: right-hand side implementations}
